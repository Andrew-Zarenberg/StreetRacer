...
public class LexusHS extends Car {
  public static final int LENGTH = 85;
  public static final int WIDTH = 41;
  
  private Image _image = (new ImageIcon(this.getClass().getResource("lexusHS.gif"))).getImage();
  
  
  public LexusHS(int x, int y, int laneWidth){
    super(x, y, LENGTH, WIDTH, laneWidth);
    _carType = "Lexus HS";
  }
  
  
  public LexusHS(int x, int y, double angle, int speed, int desiredSpeed, int lane, int laneWidth){
  ...
  public void draw(Graphics2D g2){
    ...
    if(_signalOn){
      ...
      if(_signalRight){
        ...
        g2.fillPolygon(new int[]{x+_width-4,x+_width-1 ,x+_width-4 ,x+_width-7},
                       new int[]{y+1,y+9,y+9,y+1},
                       4);
      }
    }
    ...
    if(_lights || (_police && _sirenPhase%2 == 1)){
      g2.setColor(Color.white);
      g2.fillPolygon(new int[]{x+_width-8,x+_width-5 ,x+_width-8 ,x+_width-12},
                     new int[]{y+1,y+9,y+9,y+1},
                     4);
        
      if(_lights){
        g2.setColor(new Color(255,255,255,40));
        g2.fillPolygon(new int[]{x+_width-5,x+_width+20,x+_width-30,x+_width-12},
                       new int[]{y+9,y-60,y-60,y+1},
                       4);
      }
    }
